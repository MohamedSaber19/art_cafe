...
/**
 * Generated class for the CheckoutclubPage page.
 *
...
@Component({
  selector: 'page-checkoutclub',
  templateUrl: 'checkoutclub.html',
})
export class CheckoutclubPage {
  myToken: any;
  selectedDate: string;
  myBooking: any;
  checkedIdx=0;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad CheckoutclubPage');
    this.storage.get('user_info').then((val)=>{
      this.myToken=JSON.parse(val).token;
    })
  }
  book(c){
    // this.http.get('http://artcafe.bit68.com/book_club/?token=' + this.myToken + '&club_id=' + c.id + '&selected_date=' + this.selectedDate).map(res => res.json()).subscribe(data => {
    //   console.log(data);
